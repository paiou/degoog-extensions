...
export const engine = {
  ...
  async executeSearch(
    ...
    try {
      ...
      if (timeFilter && FRESHNESS_MAP[timeFilter]) {
      ...
      for (const item of rawItems) {
        ...
        const snippet = cleanText(item.desc || item.desc_short || "")
        const rawThumb =
          item.media?.[0]?.pict?.url ||
          item.media?.[0]?.pict_big?.url ||
          item.thumbnail ||
          item.favicon ||
          undefined
        ...
        results.push({
          ...
          url: rawUrl,
          snippet,
          source: item.press_name
            ? `Qwant (${cleanText(item.press_name)})`
